...
- **Verification Status:** Unverified approximate computation. The module is intended to perform approximate pruning checks; however, this computation is unverified and not formally proven to be sound. While the implementation aims to round values conservatively to avoid overestimating abundances, it is treated as intent-only and does not carry any formal soundness or correctness guarantees. The exact Chinese Remainder Theorem (CRT) and Touchard checks remain authoritative for correctness when reached. Note that once a branch is pruned by the LLL prune decision, it is permanently pruned and cannot be recovered by the subsequent exact checks.
