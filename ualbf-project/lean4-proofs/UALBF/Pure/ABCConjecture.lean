...
/-- radical of a natural number computed strictly using exact prime factorization. -/
def radical (n : ℕ) : ℕ :=
...
def ABCConjectureStatement (ε : ℚ) (K : ℚ) : Prop :=
  ∀ (a b c : ℕ), a > 0 → b > 0 → Nat.Coprime a b → a + b = c →
    let num := (1 + ε).num.natAbs
    let den := (1 + ε).den
    (c : ℚ) ^ den ≤ K ^ den * (radical (a * b * c) : ℚ) ^ num
...
theorem derive_conjectural_ceiling
    ...
    (h_bound : K ^ (1 + ε).den * (radical (a * b * c) : ℚ) ^ (1 + ε).num.natAbs ≤ (10^30 : ℚ) ^ (1 + ε).den) :
    (c : ℚ) ^ (1 + ε).den ≤ (10^30 : ℚ) ^ (1 + ε).den := by
