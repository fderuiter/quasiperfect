...
def test_guarded_rebuild_success():
    ...
    2. The build proceeds beyond lake check (even if it eventually fails on missing files,
       the custom 'FATAL: Lean proof verification failed!' panic is NOT triggered).
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # The dummy file and directory should have been purged by the build script
        assert not dummy_file.exists(), "The intermediate C-IR dummy file was not purged!"
...
def test_guarded_rebuild_failure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Verify our detailed diagnostic paths and exact rerun command are in stderr
        assert "FATAL: Lean proof verification failed!" in res.stderr
        assert "Proof Logs / Build Directory:" in res.stderr
        assert "To troubleshoot and rerun the verification manually, execute:" in res.stderr
