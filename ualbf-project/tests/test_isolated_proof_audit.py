...
def test_proof_audit_staging_workspace_immutability():
    ...
    def mock_subprocess_run(cmd, *args, **kwargs):
        if isinstance(cmd, list) and len(cmd) > 0 and cmd[0] in ("lean", "lake", "cargo", "make"):
            return mock.Mock(returncode=0, stdout="depends on axioms: [propext, Classical.choice, Quot.sound]\n", stderr="")
        return mock.Mock(returncode=0, stdout="", stderr="")

    auditor._setup_staging_workspace = hook_setup

    try:
        with mock.patch("auditor.subprocess.run", side_effect=mock_subprocess_run), mock.patch(
