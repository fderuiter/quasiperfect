...
class TestUnifiedHelpersAndSharedValidation:
    def test_explored_ranges_parsing(self):
        """Verify that explored_ranges is supported and correctly parsed by verify_telemetry_paths."""
        from verify_cert import verify_telemetry_paths

        # Build certificate with explored_ranges
        cert = {
            "telemetry": {
                "explored_ranges": [
                    {"start_bound": [], "end_bound": [5]},
                    {"start_bound": [5], "end_bound": []},
                ]
            }
        }

        # If verify_telemetry_paths runs without error/exit, it is successfully validated!
        verify_telemetry_paths([cert])

    def test_explored_ranges_missing_or_invalid_fails(self):
        """Verify that if none of path_ranges, inner_paths, or explored_ranges exist, it fails."""
        from verify_cert import verify_telemetry_paths

        cert_missing = {"telemetry": {}}
        with pytest.raises(SystemExit):
            verify_telemetry_paths([cert_missing])

        cert_not_list = {"telemetry": {"explored_ranges": "not a list"}}
        with pytest.raises(SystemExit):
            verify_telemetry_paths([cert_not_list])

    def test_hash_file_native(self, tmp_path):
        """Verify that cert_util.hash_file delegating to Rust correctly hashes a physical file."""
        import cert_util
        ...
        # Calculate expected hash in Python
        import hashlib

        expected_hash = hashlib.sha256(test_content).hexdigest()

        # Calculate with native helper
        computed_hash = cert_util.hash_file(str(test_file))
